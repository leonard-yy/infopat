...
layui.define(["element", "jquery", "layer", "loader"], function (exports) {
  ...
  layuimini = new (function () {
    ...
    this.init = function (url) {
      //   var loading = layer.load(0, { true: false, time: 2 * 1000 });\
          ...
          var addMenuClass = function ($element, type) {
            if (type == 1) {
              ...
              if ($element.attr("class") != "layui-nav-item layui-this") {
                ...
                $(".layui-left-nav-tree").attr(
                  "class",
                  "layui-nav layui-nav-tree layui-left-nav-tree layui-hide"
                );
                $("#" + moduleId).attr(
                  "class",
                  "layui-nav layui-nav-tree layui-left-nav-tree layui-this"
                );
              }
            } else {
          ...
          for (var i = pageHeader.length - 1; i >= 0; i--) {
            ...
            if (i != 0) {
              pageHeaderHtml +=
                "<a><cite>" +
                pageHeader[i] +
                '</cite></a><span lay-separator="">/</span>\n';
            } else {
    ...
    this.initBgColor = function () {
      ...
      var styleHtml =
        ".layui-layout-admin .layui-header{background-color:" +
        bgcolorData.headerRight +
        "!important;}\n" +
        ".layui-header>ul>.layui-nav-item.layui-this,.layuimini-tool i:hover{background-color:" +
        bgcolorData.headerRightThis +
        "!important;}\n" +
        ".layui-layout-admin .layui-logo {background-color:" +
        bgcolorData.headerLogo +
        "!important;}\n" +
        ".layui-side.layui-bg-black,.layui-side.layui-bg-black>.layui-left-menu>ul {background-color:" +
        bgcolorData.menuLeft +
        "!important;}\n" +
    ...
    this.initMenu = function (data) {
      ...
      $.each(data, function (key, val) {
        ...
        $.each(menuList, function (index, menu) {
          let active = menu.active == "true" ? "layui-this" : "";
          leftMenuHtml += '<li class="layui-nav-item ' + active + '">\n';
            ...
            var buildChildHtml = function (html, child, menuParameId) {
              ...
              $.each(child, function (childIndex, childMenu) {
                ...
                if (childMenu.child != undefined && childMenu.child != []) {
                  ...
                  html +=
                    '<a href="javascript:;" class="layui-menu-tips" data-type="tabAdd"  data-one-page-mpi="m-p-i-' +
                    menuParameId +
                    '" data-one-page="' +
                    childMenu.href +
                    '" target="' +
    ...
    this.buildBgColorHtml = function () {
      ...
      $.each(bgColorConfig, function (key, val) {
        ...
        html +=
          ...
          '<div><span style="display:block; width: 20%; float: left; height: 12px; background: ' +
          val.headerLogo +
          ';"></span><span style="display:block; width: 80%; float: left; height: 12px; background: ' +
          val.headerRight +
          ';"></span></div>\n' +
          '<div><span style="display:block; width: 20%; float: left; height: 40px; background: ' +
          val.menuLeft +
  ...
  $("body").on("click", "[data-menu]", function () {
    ...
    // header
    $(".layui-header-menu .layui-nav-item.layui-this").removeClass(
      "layui-this"
    );
    $(this).addClass("layui-this");
    // left
    $(".layui-left-menu .layui-nav.layui-nav-tree.layui-this").addClass(
      "layui-hide"
    );
    $(
      ".layui-left-menu .layui-nav.layui-nav-tree.layui-this.layui-hide"
